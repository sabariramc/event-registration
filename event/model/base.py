...
def execute_sql_statement(sql_stmt: str, parameters: dict = None, is_insert=False, print_sql=None):
    """
    ...
    if isinstance(parameters, dict):
        for key, value in parameters.items():
            if isinstance(value, dict) or isinstance(value, list):
                serialized_obj = self.get_json_serialized_obj(value)
    ...
    try:
        ...
        if error_flag:
            query_execution_summary = {
                "query": sql_stmt if print_sql is None else print_sql
                , "objectType": "sqlQueryExecutionSummary"
                , "status": "failure"
                , "parameters": parameters
            }
            self.log_error("Query Execution Failed")
            self.print_query_execution_summary(query_execution_summary, LOGLevel.ERROR)
    ...
    self.log_info("Query Execution Summary")
    self.print_query_execution_summary(query_execution_summary, LOGLevel.INFO)
    if len(out) > 0:
        self.log_info("Query Result", out)
    return out
